...
import { Bell, CircleAlert, DollarSign, Users } from 'lucide-react'
    ...
    className='fixed inset-0 z-10 bg-emerald-700 flex flex-col'>
               <div className='flex items-center justify-between gap-3 text-emerald-400 h-[100px] p-8 border-b-[1px] border-b-gray-400'>
                ...
                <div className='text-white font-mont p-4 flex flex-col gap-3 max-h-[70vh] overflow-y-auto'>
                     <span onClick={()=>{setCurrentPage("Dashboard"); setShowMenu(false)}} className={`flex items-center gap-2 text-md p-2 ${currentPage === "Dashboard" ? "bg-white/20" : ""} rounded-lg cursor-pointer hover:bg-white/20 transition-bg duration-300 ease-in-out`} ><MdOutlineDashboard className='min-w-[15px] min-h-[15px]' /> Dashboard</span>
            <span onClick={()=>{setCurrentPage("Users"); setShowMenu(false)}} className={`flex items-center gap-2 text-md p-2 ${currentPage === "Users" ? "bg-white/20" : ""} rounded-lg cursor-pointer hover:bg-white/20 transition-bg duration-300 ease-in-out`} ><Users size={16} /> Users</span>
            <span onClick={()=>{setCurrentPage("Disputes"); setShowMenu(false)}} className={`flex items-center gap-2 text-md p-2 ${currentPage === "Disputes" ? "bg-white/20" : ""} rounded-lg cursor-pointer hover:bg-white/20 transition-bg duration-300 ease-in-out`} ><CircleAlert size={16} /> Disputes</span>
           <span onClick={()=>{setCurrentPage("Finance"); setShowMenu(false)}} className={`flex items-center gap-2 text-md p-2 ${currentPage === "Finance" ? "bg-white/20" : ""} rounded-lg cursor-pointer hover:bg-white/20 transition-bg duration-300 ease-in-out`} ><DollarSign size={16} /> Finance</span>
           <span onClick={()=>{setCurrentPage("Settings"); setShowMenu(false)}} className={`flex items-center gap-2 text-md p-2 ${currentPage === "Settings" ? "bg-white/20" : ""} rounded-lg cursor-pointer hover:bg-white/20 transition-bg duration-300 ease-in-out`} ><IoSettingsOutline className='min-w-[15px] min-h-[15px]' /> Settings</span>
