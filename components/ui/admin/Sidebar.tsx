  ...
  return (
    <div className='hidden h-screen bg-emerald-700  w-[15vw] min-w-[160px] max-w-[320px] md:flex flex-col border-black'>
        ...
        <div className='text-white font-mont p-4 flex flex-col gap-3 max-h-[70vh] overflow-y-auto'>
            ...
            <span onClick={()=>setCurrentPage("Users")} className={`flex items-center gap-2 text-md p-2 ${currentPage === "Users" ? "bg-white/20" : ""} rounded-lg cursor-pointer hover:bg-white/20 transition-bg duration-300 ease-in-out`} ><Users size={16} /> Users</span>
            <span onClick={()=>setCurrentPage("Disputes")} className={`flex items-center gap-2 text-md p-2 ${currentPage === "Disputes" ? "bg-white/20" : ""} rounded-lg cursor-pointer hover:bg-white/20 transition-bg duration-300 ease-in-out`} ><CircleAlert size={16} /> Disputes</span>
            <span onClick={()=>setCurrentPage("Finance")} className={`flex items-center gap-2 text-md p-2 ${currentPage === "Finance" ? "bg-white/20" : ""} rounded-lg cursor-pointer hover:bg-white/20 transition-bg duration-300 ease-in-out`} ><DollarSign size={16} /> Finance</span>
